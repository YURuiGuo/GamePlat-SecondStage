...
public class ChessChain implements Serializable {
    ...
    private int size;//气链棋盘的大小，与chessboard大小同步
    private int index=0;//气链编号。每产生一个气链，则此index+1
    private int[][] chainboard = null;//记录各个棋子所示得到气链编号
    private int[] chainfate = new  int[181*3];// 气链对应的气数
    ...
    public int updatechainboard(int x, int y,int[][] chessboard){
        int fate = 4;//x*y == size-2||x*y==1||x*y == (size-2)*(size-2)
    ...
    public boolean captureUpdate(int x,int y, int[][] chessboard){//发生提子后更新气数。由于上述得到逻辑是将所有的棋子归为一个Chain，所以只需要提对应的chain即可。
        //获取Fate为0的邻居
    ...
    public void captureAddFate(int x, int y){
        int cindex = chainboard[x][y];

        for(int i=1;i<=size-2;i++){
            for (int j = 1; j <= size - 2; j++) {
                if(chainboard[i][j] == cindex){//被提起的棋子，检查该棋子的邻居，为它的邻居加气
                    int[] dr = {-1, 1, 0, 0};
                    int[] dc = {0, 0, -1, 1};
                    for (int t = 0; t < 4; t++) {
                        int newRow = x + dr[t];
                        int newCol = y + dc[t];
                        if (isValidPosition(newRow, newCol,chainboard)&&chainboard[newRow][newCol]>=0&&chainboard[newRow][newCol]!=cindex) {
                            chainfate[chainboard[newRow][newCol]]++;
                        }
                    }
                }
                    ...
                    if(!flag){//还没有被并到新链中
                        ...
                        for(int q=1;q<=size-2;q++){
                            for(int p=1;p<=size-2;p++){
                                if(asChainB[q][p]==index2){
                                    asChainB[q][p] = index1;
                                }
                            }
                        }
                        asChainfate[index1]=asChainfate[index1]+ asChainfate[index2] -1;
                        asChainfate[index2] = -1;//使chainfate[index2]失效。
